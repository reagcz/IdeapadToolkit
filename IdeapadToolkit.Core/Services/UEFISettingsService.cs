using IdeapadToolkit.Core.Helpers;
using Serilog;
    ...
    public class UEFISettingsService : IUEFISettingsService
    {
        ...
        private bool SetPrivilege(bool enable)
        {
            try
            {
                ...
                if (!Win32.AdjustTokenPrivileges(zero, false, ref newState, 0, nint.Zero, nint.Zero))
                {
                    return false;
                }
            }
        ...
        public bool GetFlipToBootStatus()
        {
            ...
            try
            {
                ...
                var res = Win32.GetFirmwareEnvironmentVariableExW(ScopeName, Guid, ref structure, Marshal.SizeOf<LenovoFlipToBootSwInterface>(), nint.Zero);
                if (res != 0)
                {
                    dataFromUefi = structure.FlipToBootEn;
                }
                else
                {
